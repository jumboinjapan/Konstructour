...
// 2) Build fields - ТОЛЬКО СУЩЕСТВУЮЩИЕ ПОЛЯ В ТАБЛИЦЕ
// Используем только те поля, которые точно есть в таблице городов
...
if (!empty($created['id'])) {
  ...
  // Поле Region не существует - проверяем только связанную запись
  $linkedOk = $regionLinked;
}
